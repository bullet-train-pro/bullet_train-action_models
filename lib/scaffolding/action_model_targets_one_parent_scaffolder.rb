require_relative "action_model_targets_one_parent_transformer"

module ActionModelTargetsOneParentScaffolder
  def display_action_model_targets_one_parent_help
    puts ""
  ...
  def scaffold_action_model_targets_one_parent(args)
    ...
    output = `bin/rails g model #{transformer.transform_string("Scaffolding::CompletelyConcrete::TangibleThings::TargetsOneParentAction")} #{transformer.transform_string("absolutely_abstract_creative_concept")}:references started_at:datetime completed_at:datetime target_count:integer performed_count:integer scheduled_for:datetime sidekiq_jid:string created_by:references approved_by:references`
